...
void SineWave3D::initSineWave()
{
	...
	for(int j = 0; j <= tess; j++)
	{
		for(int i = 0; i <= tess; i++)
		{
			...
			//vertices[loop].setY(A1 * sin(k1 * vertices[loop].x + w1 * time));
			switch(waveDim)
			{
				case 2:
					vertices[loop].y =(A1 * sinf(k1 * vertices[loop].x + w1 * time));
					//normal normal calucation
					loop++;
					vertices[loop].x =(-A1 * k1 * cosf(k1* vertices[loop-1].x + w1 * time));
					vertices[loop].y =(1.0);
					vertices[loop].z =(0.0);
					break;
				case 3:
					vertices[loop].y =(A1 * sinf(k1 * vertices[loop].x + w1 * time) + A2 * sinf(k2 * vertices[loop].z + w2 * time));
					//normal calucation
					loop++;
					vertices[loop].x =(-A1 * k1 * cosf(k1* vertices[loop-1].x + w1 * time));
					vertices[loop].z =(-A2 * k2 * cosf(k2 * vertices[loop-1].z + w2 * time));
					vertices[loop].y =(1.0);
				default:
					break;
...
**/
	...
	glm::vec3 * ptr = (glm::vec3 *)glMapBuffer(GL_ARRAY_BUFFER,GL_READ_WRITE);
	...
	{
		//vertices[loop].y =(A1 * sin(k1 * vertices[loop].x + w1 * time));
		switch(waveDim)
		{
			case 2:
				ptr[i].y =(A1 * sinf(k1 * ptr[i].x + w1 * time));
				i++;
				ptr[i].x =(-A1 * k1 * cosf(k1 * ptr[i - 1].x + w1 * time));
				ptr[i].y =(1.0);
				ptr[i].z =(0.0);
				break;
			case 3:
				ptr[i].y =(A1 * sinf(k1 * ptr[i].x + w1 * time) + A2 * sinf(k2 * ptr[i].z + w2 * time));
				i++;
				ptr[i].x =(-A1 * k1 * cosf(k1* ptr[i -1].x + w1 * time));
				ptr[i].z =(-A2 * k2 * cosf(k2 * ptr[i -1].z + w2 * time));
				ptr[i].y =(1.0);
			default:
				break;
		}
...
//this code can work for AMD and Nvida as well
	...
	for(int i = 0; i < size; i++)
	{
			...
			{
				case 2:
					vertices[i].setY(A1 * sinf(k1 * vertices[i].x + w1 * time));
					i++;
					vertices[i].setX(-A1 * k1 * cosf(k1 * vertices[i - 1].x + w1 * time));
					vertices[i].setY(1.0);
					vertices[i].setZ(0.0);
					break;
				case 3:
					vertices[i].setY(A1 * sinf(k1 * vertices[i].x + w1 * time) + A2 * sinf(k2 * vertices[i].z + w2 * time));
					i++;
					vertices[i].setX(-A1 * k1 * cosf(k1* vertices[i -1].x + w1 * time));
					vertices[i].setZ(-A2 * k2 * cosf(k2 * vertices[i -1].z + w2 * time));
					vertices[i].setY(1.0);
				default:
					break;
			}
...
void SineWave3D::initGlenBuffers()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, buffers[0]);
		//GL_DYNAMIC_DRAW
    glBufferData(GL_ARRAY_BUFFER, size * sizeof(glm::vec3), vertices, GL_DYNAMIC_DRAW);
    int bufferSize = 0;
...
void SineWave3D::drawNormVec()
{
	...
	for(int i = 0; i < size; i++)
	{
		...
		glVertex3f(vertices[i-1].x + x , vertices[i-1].y + y, vertices[i-1].z + z);
	}
	#endif
...
void SineWave3D::printDebug()
{
	...
	for(int i = 0; i < size; i ++)
	{
		std::cout << "Vertices[" << i << "]s value is " << vertices[i].x  << " "<<
			vertices[i].y << " " << vertices[i].z << " " << std::endl;
	}
	
	for(int i = 0; i < size; i ++)
	{
		std::cout << "normalVertices[" << i << "]s value is " << normalVertices[i].x  << " "<<
			normalVertices[i].y << " " << normalVertices[i].z << " " << std::endl;
	}
...
{
	glBindBuffer(GL_ARRAY_BUFFER, buffers[0]);
	glVertexPointer(3, GL_FLOAT, 2 * sizeof(glm::vec3), NULL);

	if(lighting)
	{
		glNormalPointer(GL_FLOAT, 2 * sizeof(glm::vec3), ((glm::vec3*)NULL) + 1);
	}
	glEnableClientState(GL_VERTEX_ARRAY);
